...
class PULSE(paddle.nn.Layer):
    ...
    def forward(self, ref_im,
        ...
        if(tile_latent):
            ...
            #latent = paddle.randn(
            #    (batch_size, 1, 512), dtype=paddle.float32, requires_grad=True, device='cuda')
        else:
            ...
            #latent = paddle.randn(
            #    (batch_size, 18, 512), dtype=paddle.float32, requires_grad=True, device='cuda')
        #paddle.save(latent,"latent.Path")
        #codela=paddle.load("latent.Path")
        #latent=codela
        #latent.stop_gradient=False


        # Generate list of noise tensors
        noise = [] # stores all of the noise tensors
        noise_vars = []  # stores the noise tensors that we want to optimize on
        #pnoise=paddle.load("noise.Path")
        for i in range(18):
        ...
        for j in range(steps):
            ...
            if(loss < min_loss):
                ...
                [f'{x}: {y}' for x, y in loss_dict.items()])
            ...
            scheduler.step()
            #print(latent_in)
        #print(self.synthesis.state_dict())
        ...
        if(min_l2 <= eps):
            yield (paddle.clip(gen_im.clone().cpu().detach(),0, 1),paddle.clip(loss_builder.D(best_im).cpu().detach(),0, 1))
        else:
