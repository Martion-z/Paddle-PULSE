    ...
    if(kwargs["save_intermediate"]):
        ...
        for j,(HR,LR) in enumerate(model(ref_im,**kwargs)):
            for i in range(kwargs["batch_size"]):
                ...
                gen_lr = LR[i].cpu().detach().clamp(0, 1).numpy().transpose((1, 2, 0))*255
                gen_lr = Image.fromarray(gen_lr.astype(np.uint8))
                gen_lr.save(int_path_LR / f"{ref_im_name[i]}_{j:0{padding}}.png")
