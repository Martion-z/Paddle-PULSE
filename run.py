...
#I/O arguments
parser.add_argument('-input_dir', type=str, default='images/input', help='input data directory')
parser.add_argument('-output_dir', type=str, default='images/output1024', help='output data directory')
    ...
    if(kwargs["save_intermediate"]):
        ...
        for j,(HR,LR) in enumerate(model(ref_im,**kwargs)):
            for i in range(kwargs["batch_size"]):
                ...
                ref = np.array(out_hr.convert('LA'))[:,:,0]
                sum_niqe = sum_niqe + niqe(ref)
                count +=1
average_n=sum_niqe/count
print('the average NIQE of the best generate image is: %0.3f'% average_n)
