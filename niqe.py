...
def niqe(inputImgData):
    ...
    params = scipy.io.loadmat(join(module_path, './', 'modelparameters.mat'))
    ...
    niqe_score = (np.sqrt(np.dot(np.dot(X, pinvmat), X)))/2
    ...
    #ref = np.array(Image.open('output2/000229.png').convert('LA'))[:,:,0] # ref
    #dis = np.array(Image.open('./test_imgs/bikes_distorted.bmp').convert('LA'))[:,:,0] # dis

    #print('NIQE of ref image is: %0.3f'% niqe(ref))
    #print('NIQE of dis bikes image is: %0.3f'% niqe(dis))

    #ref = np.array(Image.open('t2.jpg').convert('LA'))[:,:,0] # ref
